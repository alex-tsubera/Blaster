...
void UMenu::MenuSetup(int32 NumOfPublicConnections, FString TypeOfMatch, FString PathToLobby)
{
	...
	if (MultiplayerSessionsSubsystem)
	{
		...
		MultiplayerSessionsSubsystem->MultiplayerOnDestroysSessionComplete.AddDynamic(this, &ThisClass::OnDestroySession);
...
void UMenu::OnFindSessions(const TArray<FOnlineSessionSearchResult>& SessionResults, bool bWasSuccessful)
{
	...
	if (!bWasSuccessful)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString(TEXT("No sessions found")));
		}
		return;
	}
		...
		if (SettingsValue == MatchType)
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, 
					FString::Printf(TEXT("Joining to user %s"), 
...
void UMenu::OnJoinSession(EOnJoinSessionCompleteResult::Type Result)
{
	if (Result != EOnJoinSessionCompleteResult::Success)
	{
		...
		if (GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString::Printf(TEXT("Failed to join session (%s)"), LexToString(Result)));
		}
		return;
	}
